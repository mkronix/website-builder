...
const getComponentName = (reactCode: string): string => {
    const functionMatch = RegExp(/function\s+(\w+)/).exec(reactCode);
    if (functionMatch) return functionMatch[1];

    const arrowMatch = RegExp(/const\s+(\w+)\s*=/).exec(reactCode);
